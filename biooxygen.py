...
def plots(samples, param, flow, flow_inv):#, sam, sam2):
    ...
    fig, ax = plt.subplots(1, 2, figsize=(11, 4), sharex=False, sharey=False)
...
def main(args):
    ...
    *algos, flows = fm_run(dist, args, N_PARAM, optim, prior_gn)
    plots(*algos[0], *flows)
    plt.savefig("Algorithm 2 adjoint gradients.png", bbox_inches='tight')
    # plots(*algos[1], *flows)
    # plt.savefig("Algorithm 2 discretize-then-optimize.png", bbox_inches='tight')
    plots(*algos[2], *flows)
    plt.savefig("Algorithm 3 refresh.png", bbox_inches='tight')
    # plots(*algos[3], *flows)
    # plt.savefig("Algorithm 3 no refresh.png", bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    parser.add_argument('-ss', '--step-size', type=float, nargs=3, default=[0.15, 0.5, 0.4])
    parser.add_argument('-si', '--sample-iter', type=int, default=1)
